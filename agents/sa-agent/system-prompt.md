...
### ЗАПРЕЩЁННЫЕ СЛОВА (валидация автоматически отклонит артефакт)
- НЕ пиши: бронь, заказ → пиши: запись (booking)
- НЕ пиши: специалист, работник, сотрудник → пиши: мастер (master)
- НЕ пиши: пользователь, заказчик → пиши: клиент (client) или владелец (owner)
- НЕ пиши: хозяин, бизнесмен → пиши: владелец (owner)
- Это касается ВСЕГО текста включая вопросы, примечания и комментарии.
